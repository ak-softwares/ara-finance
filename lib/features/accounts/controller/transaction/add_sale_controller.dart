...
class AddSaleTransactionController extends GetxController {
  static AddSaleTransactionController get instance => Get.find();
  ...
  RxList<ImageModel> saleInvoiceImages = <ImageModel>[].obs;
  ...
  Future<void> pickImage() async {
    ...
    if (pickedImage != null) {
      saleInvoiceImages.add(ImageModel(image: pickedImage));
    }
  }
    ...
    try {
      ...
      if (image.image != null) {
        ...
        if (fetchImage.imageUrl != null) {
          int index = saleInvoiceImages.indexWhere((img) => img.imageId == image.imageId);
          if (index != -1) {
            saleInvoiceImages[index].imageUrl = fetchImage.imageUrl;
            saleInvoiceImages[index].imageId = fetchImage.imageId;
            saleInvoiceImages.refresh();
          }
  ...
  Future<void> deleteImage(ImageModel image) async {
    try {
      ...
      saleInvoiceImages.removeWhere((img) => img.image == image.image);
    } catch (e) {
  ...
  CartModel convertProductToCart({required ProductModel product, required int quantity, int variationId = 0}) {
    return CartModel(
      ...
      subtotal: (quantity * (product.price ?? 0)).toStringAsFixed(0),
      total: (quantity * (product.price ?? 0)).toStringAsFixed(0),
      subtotalTax: '0',
      totalTax: '0',
      sku: product.sku,
      ...
      vendor: product.vendor?.id == null ? selectedCustomer.value : null,
    );
  }
  ...
  void saveSaleTransaction() {
    TransactionModel transaction = TransactionModel(
      ...
      formAccountVoucher: selectedSaleVoucher.value,
  ...
  void resetValue(TransactionModel transaction) {
    ...
    selectedSaleVoucher.value = transaction.formAccountVoucher ?? AccountVoucherModel();
